...
def preprocessing_keras(df, tokenizer=None) -> list:
    ...
    if not tokenizer:
        ...
        X, X_val = pad_sequences(X, maxlen=MAX_REVIEW_LENGTH), pad_sequences(
            X_val, maxlen=MAX_REVIEW_LENGTH)
...
def initial_model():
    ...
    embedding_layer = Embedding(input_dim=NUM_MOST_FREQ_WORDS,
...
def main_dl():
    ...
    gru_model.fit(X, y, batch_size=128, epochs=5,
                  validation_data=(X_val, y_val), callbacks=[es, cp])
    # weight with minimum validataion loss
    gru_model.load_weights('./checkpoint')
